...
const TopPhoto = () => {
    ...
    const fetchData = async () => {
        try {
            const trendingData = await Axios.get('trending/tv/week');
            const showData = await Axios.get(`/tv/${trendingData.data.results[num].id}`);
            setData(showData);
            const words = showData.data.overview.split(' ');
            if (words.length > 50) {
                setOverView(words.slice(0, 50).join(' ') + '...');
            } else {
                setOverView(showData.data.overview);
            }
        } catch (err) {
            console.log(err);
        }
    ...
    const handlePlayClick = () => {
        // Navigate to WatchTV with the required parameters
        navigate(`/watch/tv/${data.data.id}`); // Adjust path if needed
    };

    const handleMoreInfoClick = () => {
        // Open the original TMDB path for the TV show
        window.open(`https://www.themoviedb.org/tv/${data.data.id}`, '_blank'); // Open in a new tab
    };
                ...
                >
                    ...
                    >
                        ...
                        <Typography sx={{
                            ...
                            {data.data.name} {/* Changed to name for TV show */}
                        </Typography>
                        <Typography sx={{
                            color: 'grey.400',
                            fontSize: {
